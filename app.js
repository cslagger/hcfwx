...
window.openPrintView = async function() {
            ...
            if(hfData[key] && hfData[key].length > 0) {
                ...
                hfData[key].forEach(line => {
                    ...
                    if (line.includes(':')) {
                        ...
                        // Route: Clean & Remove "Air Traffic Control"
                        let route = line.substring(0, colonIdx).replace(/AIR TRAFFIC CONTROL/gi, '').trim();
                        // Frequencies: Split by pipe, join with slash
                        let freqs = line.substring(colonIdx + 1).split('|').map(s => s.trim()).join(' / ');
                        
                        html += `<div style="margin-bottom:6px; break-inside: avoid;">
                            <div style="font-size:10px; color:#555; font-weight:bold; text-transform:uppercase;">${route}</div>
                            <div style="font-size:14px; color:#000; font-weight:900;">${freqs}</div>
                        </div>`;
                    } else {
                        // Notes handling: "Notes; Content"
                        // Clean any existing prefix like "Notes" or "Note:" to avoid duplications
                        let cleanLine = line.replace(/^NOTES?[:\s-]*/i, '').trim();
                        if(cleanLine) {
                            html += `<div style="grid-column: span 2; font-style:italic; margin-top:4px; font-size:11px;">Notes; ${cleanLine}</div>`;
                        }
                    }
                });
            }
    ...
    function renderGroup(title, fixList, hfKeys) {
        ...
        if (hfContent) {
            html += `<div style="margin-bottom:10px; border:1px solid #444; padding:8px; font-size:11px; background:#f9f9f9;">
                <div style="font-weight:bold; border-bottom:1px solid #ccc; margin-bottom:4px;">PACIFIC HF ASSIGNMENTS (Hawaii Departures Only)</div>
                ${hfContent}
            </div>`;
        }
    ...
    // Page 1: Sector 7/8
    htmlContent += renderGroup("HCF Oceanic Winds Aloft Forecast - Sector 7/8", sector1, ["hwn_cal_major", "hwn_cal_other", "hwn_pacnw", "hwn_alaska", "hwn_south", "notes"]);
